...
async function onRequestGet2({ request, env }) {
  ...
  const metaRes = await fetch(
    `${SUPABASE_URL2}/rest/v1/md_series?id=eq.${encodeURIComponent(seriesId)}&select=price,free_eps,hidden`,
...
async function onRequestGet4({ request, env, params }) {
  const file = Array.isArray(params.path) ? params.path.join("/") : String(params.path || "");
...
// ../.wrangler/tmp/pages-AUV0FF/functionsRoutes-0.9341384962033598.mjs
var routes = [
  {
